...
from .models import Carteira, Transacao, Categoria
...
@router.get("/dashboard/resumo", response=DashboardResumo)
def get_dashboard_resumo(request):
    ...
    hoje = date.today()
    transacoes_mes = Transacao.objects.filter(user=user, data__year=hoje.year, data__month=hoje.month)
...
@router.get("/dashboard/graficos", response=DashboardGraficos)
def get_dashboard_graficos(request):
    ...
    def agrupar_por_categoria(tipo_transacao):
        dados = Transacao.objects.filter(user=user, tipo=tipo_transacao).values('categoria__nome').annotate(total=Sum('valor')).order_by('-total')
        resultado = []
        for i, item in enumerate(dados):
            resultado.append({"name": item['categoria__nome'], "value": item['total'], "color": CORES[i % len(CORES)]})
        return resultado

    return {"receitas_por_categoria": agrupar_por_categoria('RECEITA'), "despesas_por_categoria": agrupar_por_categoria('DESPESA')}

@router.get("/transacoes", response=List[TransacaoSchema])
def listar_transacoes(request):
    ...
    for t in qs:
        resultados.append({
            "id": t.id, "descricao": t.categoria.nome, "valor": t.valor, "tipo": t.tipo, "data": t.data,
            "carteira_id": t.carteira.id, "categoria_id": t.categoria.id, "observacao": t.observacao
        })
    return resultados

@router.post("/transacoes", response={201: TransacaoSchema})
def criar_transacao(request, payload: NovaTransacaoSchema):
    ...
    nova = Transacao.objects.create(
        user=user, carteira=carteira, categoria=categoria, valor=payload.valor, data=payload.data,
        tipo=payload.tipo, pago=payload.pago, observacao=payload.observacao
    )
    return 201, {
        "id": nova.id, "descricao": nova.categoria.nome, "valor": nova.valor, "tipo": nova.tipo, "data": nova.data,
        "carteira_id": nova.carteira.id, "categoria_id": nova.categoria.id, "observacao": nova.observacao
    }

@router.put("/transacoes/{transacao_id}", response=TransacaoSchema)
def atualizar_transacao(request, transacao_id: int, payload: NovaTransacaoSchema):
    ...
    transacao.save()
    return {
        "id": transacao.id, "descricao": transacao.categoria.nome, "valor": transacao.valor, "tipo": transacao.tipo, "data": transacao.data,
        "carteira_id": transacao.carteira.id, "categoria_id": transacao.categoria.id, "observacao": transacao.observacao
    }

@router.delete("/transacoes/{transacao_id}", response={204: None})
...
@router.get("/combos/carteiras", response=List[ItemSelecao])
def listar_carteiras_combo(request):
    user = request.auth
    return Carteira.objects.filter(user=user)

@router.get("/combos/categorias", response=List[ItemSelecao])
...
@router.post("/categorias", response={201: CategoriaSchema})
def criar_categoria(request, payload: NovaCategoriaSchema):
    user = request.auth
    nova = Categoria.objects.create(user=user, nome=payload.nome, tipo=payload.tipo, icone=payload.icone)
    return 201, nova

@router.delete("/categorias/{cat_id}", response={204: None})
...
@router.get("/carteiras", response=List[CarteiraSchema])
def listar_carteiras(request):
    user = request.auth
    return Carteira.objects.filter(user=user)

@router.post("/carteiras", response={201: CarteiraSchema})
def criar_carteira(request, payload: NovaCarteiraSchema):
    user = request.auth
    nova = Carteira.objects.create(user=user, nome=payload.nome, saldo_inicial=payload.saldo_inicial, descricao=payload.descricao, cor=payload.cor)
    return 201, nova

@router.put("/carteiras/{carteira_id}", response=CarteiraSchema)
...
@router.delete("/carteiras/{carteira_id}", response={204: None})
